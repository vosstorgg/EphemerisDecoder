...
- **Дополнительные точки**: Узлы, Хирон, Лилит (Black Moon), Церера, Паллада, Юнона, Веста
...
- **Настраиваемые орбисы**: В `config/aspects.yaml`
- **Логика**: Для каждой пары планет возвращается только самый точный аспект (с наименьшим орбисом)

### 3. Расчет домов

- **Система домов**: Placidus (по умолчанию)
- **Фоллбек**: Equal House при высоких широтах (>66°), где Placidus не работает
- **Результат**: Границы 12 домов с указанием знаков; в ответе поле `house_system` (`Placidus` или `Equal`)
